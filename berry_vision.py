...
class berry(Thread):
    ...
    def run(self):
        ...
        focal_person = distance_calculator.focal_length_finder(KNOWN_DISTANCE, chosenObject["person"]["width"],
                                                               person_width_in_rf)

        """http://192.168.137.188:8080/?action=stream"""
        cap = cv.VideoCapture("http://192.168.137.188:8080/?action=stream")
        ret, frame1 = cap.read()
        ret, frame2 = cap.read()
            ...
            for d in data:
                ...
                if d[0] in chosenObject:
                    ...
                    if max(nbobjects) > 1:
                        ...
                        xservo = math.ceil((xmin + xmax) / 2)
                        yservo = math.ceil((ymin + ymax) / 2)
                        print(xservo, yservo)
                    else:
                        xservo = x
                        yservo = y
                    # finding focal length
                    focal_object = distance_calculator.focal_length_finder(KNOWN_DISTANCE, chosenObject[d[0]]["width"],
                                                                           chosenObject[d[0]]["width_in_rf"])
                    distance = distance_calculator.distance_finder(focal_object, chosenObject[d[0]]["width"], d[1])
